...
import CommunitySvg from "@/assets/svgs/story/value-community.svg"
import NeutralitySvg from "@/assets/svgs/story/value-neutrality.svg"
...
const Purpose = () => {
  ...
  return (
    <Box
      sx={{
        width: "100%",
        pt: phrase === "in-progress" ? ["4.8rem", "5.5rem", "8rem"] : ["3.2rem", "4.8rem"],
      }}
    >
      <Typography
        sx={{
      ...
      <Stack direction={isPortrait ? "column" : "row"} spacing={isPortrait ? "3.2rem" : "4.8rem"} alignItems="center">
        <CardMedia
          image={StatementSvg}
          sx={{
            width: ["100%", "calc(100% - 4rem)", "56.1rem"],
            aspectRatio: "562 / 324",
          }}
        ></CardMedia>
            ...
            <Stack direction="column" key={title}>
              <SvgIcon
                sx={{
                  width: "min-content",
                  height: "min-content",
                  transform: ["scale(0.8)", "scale(0.9698)"],
                }}
                component={icon}
                inheritViewBox
              ></SvgIcon>
