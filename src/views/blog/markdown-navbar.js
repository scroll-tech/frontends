...
export class MarkdownNavbar extends Component {
  ...
  initHeadingsId() {
    const headingId = decodeURIComponent(
      this.props.declarative ? window.location.hash.replace(/^#/, "").trim() : (window.location.hash.match(/heading-\d+/g) || [])[0],
    )

    this.state.navStructure.forEach(t => {
      const headings = document.querySelectorAll(`h${t.level}`)
      const curHeading = Array.prototype.slice.apply(headings).find(h => h.innerText.trim() === t.text.trim() && (!h.dataset || !h.dataset.id))

      if (curHeading) {
        curHeading.dataset.id = this.props.declarative ? `${t.listNo}-${t.text}` : `heading-${t.index}`
  ...
  getHeadingList() {
    ...
    this.state.navStructure.forEach(t => {
      ...
      const curHeading = Array.prototype.slice
        .apply(headings)
        .find(h => h.innerText.trim() === t.text.trim() && !headingList.find(x => x.offsetTop === h.offsetTop))
      if (curHeading) {
        headingList.push({
  ...
  updateHash(value) {
    ...
    this.updateHashTimeout = setTimeout(() => {
      window.history.replaceState({}, "", `${window.location.pathname}${window.location.search}#${value}`)
    }, 0)
  }
    ...
    const tBlocks = this.getNavStructure(this.props.source).map(t => {
      ...
      return (
        <div
          className={cls}
          onClick={evt => {
            const currentHash = this.props.declarative ? `${t.listNo}-${t.text}` : `heading-${t.index}`
