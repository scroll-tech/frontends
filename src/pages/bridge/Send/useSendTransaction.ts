...
export function useSendTransaction(props) {
  ...
  const depositETH = () => {
    ...
    if (ChainId.SCROLL_LAYER_1 === ChainIdEnum.GOERLI) {
      return networksAndSigners[ChainId.SCROLL_LAYER_1].gateway["depositETH(uint256)"](parsedAmount, gasLimit)
    }
    return networksAndSigners[ChainId.SCROLL_LAYER_1].gateway["depositETH(uint256,uint256)"](gasLimit, {
      value: parsedAmount,
    })
  }
