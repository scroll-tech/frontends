...
const Value = () => {
  return (
    <SectionWrapper dark round full sx={{ pt: "5.4rem" }}>
      <Stack direction="row" justifyContent="space-between" spacing="3rem">
        ...
        <Stack direction="column" spacing="13rem" sx={{ flex: 1, pt: "14.6rem", pb: "14rem" }}>
          {STORY_VALUES.map(({ icon, title, content }) => (
            <Stack direction="column" key={title} spacing="2.2rem">
              <SvgIcon sx={{ fontSize: "4.2rem" }} component={icon} inheritViewBox></SvgIcon>
              <Typography sx={{ fontSize: "2.4rem", fontWeight: 600, color: "primary.contrastText" }}>{title}</Typography>
              <Typography sx={{ fontSize: "2rem", color: "primary.contrastText" }}>{content}</Typography>
            </Stack>
          ))}
        </Stack>
      </Stack>
