...
const ListRow = memo(function ListRow({ listUrl, onBack }: { listUrl: string; onBack: () => void }) {
  ...
  return (
    <Row key={listUrl} align="center" padding="16px" id={listUrlRowHTMLId(listUrl)}>
      {list.logoURI ? (
        <ListLogo style={{ marginRight: "1.6rem" }} logoURI={list.logoURI} alt={`${list.name} list logo`} />
      ) : (
        <div style={{ width: "24px", height: "24px", marginRight: "1.6rem" }} />
      )}
      <Column style={{ flex: "1" }}>
        <Row>
          <Text fontWeight={isSelected ? 500 : 400} fontSize={16} style={{ overflow: "hidden", textOverflow: "ellipsis" }}>
            {list.name}
          </Text>
        </Row>
        <Row
      ...
      <PaddedColumn gap="14px">
        ...
        <Row>
          <SearchInput
            ...
            style={{ height: "4.4rem", borderRadius: 12, padding: "12px" }}
          />
