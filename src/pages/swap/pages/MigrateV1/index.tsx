...
export default function MigrateV1() {
  ...
  const [V1LiquidityBalances, V1LiquidityBalancesLoading] = useTokenBalancesWithLoadingIndicator(walletCurrentAddress ?? undefined, V1LiquidityTokens)
  const allV1PairsWithLiquidity = V1LiquidityTokens.filter(V1LiquidityToken => {
    ...
    const balance = V1LiquidityBalances[V1LiquidityToken.address]
    return balance ? (
      <V1PositionCard key={V1LiquidityToken.address} token={V1Exchanges[V1LiquidityToken.address]} V1LiquidityBalance={balance} />
    ) : null
  })
    ...
    <BodyWrapper style={{ padding: 24 }}>
      <AutoColumn gap="16px">
        ...
        {!walletCurrentAddress ? (
          ...
          <>
            <AutoRow>
              <SearchInput value={tokenSearch} onChange={handleTokenSearchChange} placeholder="Enter a token address to find liquidity" />
            </AutoRow>
            {allV1PairsWithLiquidity?.length > 0 ? <>{allV1PairsWithLiquidity}</> : <EmptyState message="No V1 Liquidity found." />}
          </>
        )}
      </AutoColumn>
    </BodyWrapper>
